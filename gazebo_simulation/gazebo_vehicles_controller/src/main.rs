...
use gz::{msgs::{actuators::Actuators, double::Double, imu::IMU, laserscan::LaserScan, model::Model, navsat::NavSat}, transport::Node};
    ...
    if let Some(mut node)=  Node::new() {
        ...
        let _= node.subscribe("osprai/imu", move |msg : IMU| {
            if let Ok(mut state) = osprai_state_imu.lock() {
                ...
                println!("Attitude: {:?}", msg.orientation);
        ...
        let _= node.subscribe("osprai/lidar", move |msg: LaserScan| {
            if let Ok(mut state) = osprai_state_lidar.lock() && let Some(lidar_state) = &mut state.lidar_state {
                ...
                lidar_state.min_horizontal_angle = msg.vertical_angle_step;
                lidar_state.horizontal_resolution = msg.angle_min;
        ...
        if let Some(mut left_arm) = node.advertise::<Double>("/osprai/command/tilt_left") &&
            ...
            let mut increment= -1.0_f64.to_radians();
            let mut thrust__increment= 10.0;
            loop {
                if let Ok(mut state) = osprai_state_cmd.lock() {
                    let mut need_reverse= false;
                    let mut idx= 0;
                    for joint in &mut state.joints_angles {
                        joint.setpoint_angle += increment;
                        joint.setpoint_angle = joint.setpoint_angle.clamp(joint.min_angle, joint.max_angle);
                        if joint.setpoint_angle == joint.max_angle || joint.setpoint_angle == joint.min_angle {
                            need_reverse = true;
                        }
                        println!("Joint {} Current Angle: {:.2}, Setpoint Angle: {:.2}", idx, joint.current_angle, joint.setpoint_angle);
                        idx += 1;
                    }
                    if need_reverse {
                    ...
                    for thruster in &mut state.thrusters {
                        thruster.setpoint_thrust += thrust__increment;
                        thruster.setpoint_thrust = thruster.setpoint_thrust.clamp(0.0, thruster.max_thrust);
                        if thruster.setpoint_thrust == 0.0 || thruster.setpoint_thrust == thruster.max_thrust {
                            need_reverse = true;
                        }
                    ...
                    if need_reverse {
                        thrust__increment = -thrust__increment; // Reverse direction
                    }
